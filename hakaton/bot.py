...
@dp.callback_query_handler(state='tarifs')
async def get_tarif_info(callback_query: types.CallbackQuery):
    if callback_query.data in tarifs.keys():
       await bot.send_photo(callback_query.message.chat.id, tarifs[callback_query.data]['photo'])
       price = tarifs[callback_query.data]['price']
       name = tarifs[callback_query.data]['name']
       url= tarifs[callback_query.data]["site_url"]
